...
class KSC_KillTask : KSC_SubjectBaseTask
{
	...
	override protected void AttachSubjectHandlers()
	{
		...
		m_pDamageManager = ScriptedDamageManagerComponent.Cast(m_pSubject.FindComponent(ScriptedDamageManagerComponent));
		IEntity child = m_pSubject.GetChildren();
		
		while (!m_pDamageManager && child)
		{
			m_pDamageManager = ScriptedDamageManagerComponent.Cast(child.FindComponent(ScriptedDamageManagerComponent));			
